...
const PlatformStatistics = () => {
  ...
  // Data for Pie Chart (Order Distribution makes logical sense for a Pie)
  const pieData = [
    {
      name: "Delivered",
      value: stats.ordersDelivered || 0,
  ...
  const PIE_COLORS = ["#10b981", "#f59e0b"]; // Success Green, Warning Amber
    ...
    <div className="w-full">
      ...
      >
        ...
        <motion.div variants={itemVariants} className="bg-base-100 shadow-xl border border-base-200/50 rounded-3xl p-6 relative overflow-hidden group">
          ...
          <h2 className="text-4xl font-black text-base-content relative z-10">${stats.totalPaymentAmount?.toFixed(2) || 0}</h2>
      ...
      >
        {/* Line Chart (Full Width Span on Large Screens) */}
        <motion.div variants={itemVariants} className="bg-base-100 shadow-xl border border-base-200/50 rounded-3xl p-6 md:p-8 lg:col-span-2">
          <h2 className="text-xl font-bold mb-8 text-base-content flex items-center gap-2">
            <span className="w-2 h-6 bg-info rounded-full"></span> Growth Metrics Line
          </h2>
          <div className="h-[350px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={overviewData} margin={{ top: 20, right: 30, left: 0, bottom: 5 }}>
        ...
        <motion.div variants={itemVariants} className="bg-base-100 shadow-xl border border-base-200/50 rounded-3xl p-6 md:p-8">
          <h2 className="text-xl font-bold mb-8 text-base-content flex items-center gap-2">
            <span className="w-2 h-6 bg-primary rounded-full"></span> Engagement Overview
          </h2>
          <div className="h-[350px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={overviewData} margin={{ top: 20, right: 30, left: 0, bottom: 5 }}>
                ...
                <Tooltip 
                  ...
                  contentStyle={{ borderRadius: '12px', border: 'none', boxShadow: '0 10px 15px -3px rgba(0, 0, 0, 0.1)' }}
                />
                <Bar dataKey="value" name="Total Count" fill="#8b5cf6" radius={[6, 6, 0, 0]} barSize={45} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </motion.div>
        ...
        <motion.div variants={itemVariants} className="bg-base-100 shadow-xl border border-base-200/50 rounded-3xl p-6 md:p-8">
          <h2 className="text-xl font-bold mb-8 text-base-content flex items-center gap-2">
            <span className="w-2 h-6 bg-secondary rounded-full"></span> Order Status Distribution
          </h2>
          <div className="h-[350px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
